...
(function () {
  ...
  var BLOCKS = {
    ...
    tileAdc:        { tileSingleton: true, name: 'Tile ADC, per rail', tech: '65nm LP CMOS', freqGHz: 1, powerMw: 0, gainDb: 0, areaMm2: 0.9, addPnFloorDbc: 0, addPnCornerHz: 0, conf: 'scaled-estimate', why: 'B5 only. Power is NOT taken from here — it is computed as FOM · 2^bits · fs from the adcFomFjConv, adcBits and adcGspsPerRail parameters, so the reader can move the assumption that decides the option. The area is the fixed cost: ~0.9 mm² for a 2–3 GS/s pipelined or time-interleaved SAR in a mature node, which is 14% of one 2.5 × 2.5 mm baseband die on its own. It used to say "most of a tile die", written before the die had a stated size; now that bbDieMm fixes it at 6.25 mm² the honest figure is smaller than that — and beside the point, because what actually breaks B5 is that the ADCs, the DACs, the SerDes and their decap are all per-TILE singletons that cannot be spread across the four dies, so they land on one together and reach 95%.' },
    ...
    bbVectorMod:    { name: 'IQ vector modulator + VGA', tech: '65nm LP CMOS', freqGHz: 1, powerMw: 6, gainDb: 0, areaMm2: 0.03, nfDb: 14, conf: 'scaled-estimate', why: 'Per-channel baseband phase/amplitude weight — the beamforming control point, and where a static LO phase offset is corrected. ONE PER CHANNEL PER RAIL, so the count follows the baseband pairing rather than being fixed here: at the default 4 IQ channels on each of 4 baseband dies that is 16 per rail and 32 per tile, and it dominates the tile power. This text said a flat "32 per tile per rail" until the 1:1 pairing halved the channel count and left it asserting twice what the model computes.' },
  ...
  var PARAMS = [
    ...
    { key: 'dieInventory', label: 'RFIC dies available', units: '-', value: 100, min: 4, max: 4000, step: 1, group: 'Array & band',
      conf: 'measured/datasheet', why: 'AN INVENTORY, NOT A DIMENSION, and it REPLACES the old "RFIC dies in the array" input. ≈100 existing 2.5 × 2.5 mm E-band dies. How many dies the array CONTAINS is not a free choice — it is tiles × taps per tile, so it is now derived and cannot be typed into disagreement with the geometry. What this number does is bound it: if the aperture and the tile pitch demand more dies than exist, that is a procurement fact the tool should state rather than a term to be balanced against. It enters no equation. The only quantity it touches is the population shortfall, which is reported and deliberately NOT applied to the array gain — under-populating a panel really would cost gain, but modelling a half-built array is a different question from choosing an architecture for a whole one.' },
  ...
  */
  ...
  function resolve(state) {
    ...
    /* ---------------------------------------------------------------- *
     * HOW MANY RFIC DIES THE ARRAY CONTAINS IS DERIVED, NOT ENTERED.
     *
     * One die per LO tap, one tap per die: the count is tiles × taps per
     * tile and nothing else. It used to be a free parameter sitting beside
     * the tile pitch and the tap count, agreeing with them at the defaults
     * only because 5×5×4 happens to be 100 — move the aperture, the pitch
     * or the taps and the three drifted apart, with the tool reduced to
     * nagging about a disagreement it could simply not have.
     *
     * The ~100-die figure did not disappear; it became what it always was,
     * an INVENTORY. It bounds the geometry rather than describing it, and
     * it enters no equation.
     * ---------------------------------------------------------------- */
    g.nDies = g.loTapsTotal;
    g.dieInventory = Math.round(g.dieInventory);
    g.diesPlaced = Math.min(g.nDies, g.dieInventory);
    g.diesShort = Math.max(0, g.nDies - g.dieInventory);
    /* What a shortfall WOULD cost, reported and not applied. An array you
       cannot finish populating really does lose this much peak gain, but
       that is a question about a half-built panel; the comparison this tool
       exists to make is between architectures for a whole one, and silently
       docking every metric by it would confuse the two. */
    g.diePopGainDb = 10 * Math.log10(Math.max(g.diesPlaced, 1) / Math.max(g.nDies, 1));
  ...
  function consistency(g, bbRes) {
    ...
    /* The die count no longer disagrees with the geometry, because it IS the
       geometry — so the only question left is procurement: does the array
       this aperture and pitch describe need more dies than exist?

       The old pair of checks is gone with the free parameter. One of them was
       wrong anyway: it warned that spare inventory "costs X dB of array gain
       against a fully populated aperture", which confuses two different
       things. Having dies left over on a shelf costs nothing; under-filling
       a panel costs gain. With the count derived the aperture is always fully
       populated at the chosen pitch, and the surviving question is whether
       you can buy it. */
    if (g.diesShort > 0) {
      out.push({
        severity: 'fail',
        message: 'This geometry needs ' + g.nDies + ' RFIC dies — ' + g.nTilesTotal + ' tiles × ' +
          Math.round(g.tapsPerTile) + ' per tile — against a stated inventory of ' + g.dieInventory +
          '. It is ' + g.diesShort + ' short, ' + (g.nDies / Math.max(g.dieInventory, 1)).toFixed(2) +
          '× over. That is a procurement limit, not a design variable: the way out is a coarser tile ' +
          'pitch, fewer taps per tile, or a smaller aperture, all of which reduce the count this ' +
          'geometry demands. Populating only ' + g.diesPlaced + ' of them would cost ' +
          Math.abs(g.diePopGainDb).toFixed(2) + ' dB of peak gain — a figure this tool REPORTS and does ' +
          'not apply, because every other number on the page describes a whole array.'
      });
    }
